...
slate.rte = {
	...
	wrapIframe: function(options) {
		options.$iframes.each(function() {
			...
			this.src = this.src;
			$('.'+options.iframeWrapperClass).unwrap();
		});
	}
};
...
function centerContentResizer(arr){
	container 	= arr.par;
	content 	= arr.chld;
	hght 		= arr.hght || 120;
	...
	function initCheck() {
		...
		$contentHeight = $(content).outerHeight() + hght;
		if ( $containerHeight < $contentHeight ) {
			$(container).css(type, $contentHeight);
		}
	}
}
...
function resizeWithPadding(arr){
	...
	hght 			= arr.hght || 120;
	type 			= arr.type || 'min-height';
	$startHeight 	= $(resThis).outerHeight();
	initCheck();
	$(window).resize(function() {
		initCheck();
	});
	function initCheck() {
		$resThisHeight 	= $(resThis).outerHeight();
		$mstrHeight 	= $(mstr).outerHeight() + hght;
		if ( $startHeight < $mstrHeight ) {
			$(resThis).css(type, $mstrHeight);
		} else {
			$(resThis).css(type, $startHeight);
		}
	}
}

function headerWidth(chld,par,el) {
	chld 		= chld;
	par 		= par;
	el 			= el;
	cls 		= 'make-mobile';
	status 		= 'Page Load';
	tooThin 	= 'does not fit';
	fits 		= 'fits';
	mob 		= 'mobile';
	padding 	= 4; // Allows more graceful change, so elements dont get crowded before change
	
	startWdth   = $(window).width();

	$(el).addClass('init-width-check');

	initWidth(startWdth)
	$(window).resize(function() {
		newWdth   = $(window).width();
		initWidth(newWdth)
	})
	
	// count=0;
	if ( typeof allWdth == 'undefined') {allWdth=0;}
	if ( typeof parWdth == 'undefined') {parWdth=0;}
	if ( typeof prevAllWdth == 'undefined') {prevAllWdth=0;}
	if ( typeof prevParWdth == 'undefined') {prevParWdth=0;}
	function initWidth(wdth) {
		scrnWdth  		= wdth;
		...
		if ( scrnWdth >= 750) {
			...
			allWdth 	= 0;
			parWdth 	= 0;
			$(par+' '+chld).each(function() { 
				item 		= $(this);
				// loop++;
				// Check up the parent tree for visibility
				itemPar1	= item.parent().is(':visible');
				itemPar2	= item.parent().parent().is(':visible');
				itemPar3	= item.parent().parent().parent().is(':visible');
				itemPar4	= item.parent().parent().parent().parent().is(':visible');
				itemPar5	= item.parent().parent().parent().parent().parent().is(':visible');
				// Main Parent Width
				newPar 		= item.parent().parent();
				newWdth 	= newPar.outerWidth();
				// Confirm Parent Visibility
				itemVis		= itemPar1 && itemPar2 && itemPar3 && itemPar4 && itemPar5;
				// Get Item width 
				itemWdth 	= item.outerWidth() + padding;
				hasWdth 	= newWdth > 0;
				// If confirmed then add item width to allWdth
				if ( itemVis) {
					allWdth += itemWdth; 
					parWdth = newWdth;
					prevAllWdth = allWdth;
			...
			// Too Thin
			if ( (allWdth >= parWdth || allWdth >= prevParWdth ) && status != tooThin ) {
				status 	= tooThin;	
				$(el).addClass(cls);
			} 
			// Fits
			if ( (allWdth < parWdth || allWdth < prevParWdth ) && status != fits ) {
				status 	= fits;
				$(el).removeClass(cls);
			} 
		// If mobile sizes
...
function rteImageGrid(arr) {
	var thisParent      			= arr ? arr.par : '.rte';
	var thisItem      				= arr ? arr.item : 'img';
	$(thisParent).each(function (thisItem) {
		theseChildren 				= $(this).children(thisItem);
		$(theseChildren).each(function () {
			innerChildren 			= $(this).children();
			checkImages(innerChildren,1);
			checkImages(innerChildren,2);
			checkImages(innerChildren,3);
			checkImages(innerChildren,4);
		})
	})
	function checkImages(el,num) {
		var el 						= el;
		var loopNum 				= num ? num : 3;
		if ( el.length == loopNum ) {
			var firstEl 			= el[0].nodeName;
			if ( firstEl.toLowerCase() == thisItem ) {
				var otherEls 		= $(firstEl).nextUntil($(':not('+thisItem+')'));
				var otherElsTotal 	= otherEls.length + 1;
				if ( otherElsTotal == loopNum) {
					gridImages(el,loopNum)
				}
				if ( loopNum == 1) {
					gridImages(el,1)
				}
		...
		function gridImages(el,total) {
			var total 				= total;
			var elArray 			= [];
			var randomNum 			= Math.floor(1000 + Math.random() * 9000);
			var imgCls 				= 'image-group-'+randomNum;
			var contCls 			= 'container-'+randomNum;
			var gridEnd				= '</div>';
			var grid				= '<div class="grid vert-top grid--rte-images '+contCls+'" />';
			var imgSize 			= 'squared';
			if (total==1) { gridWidth = ''; imgSize = ''; }
			if (total==2) { gridWidth = 'medium-up--one-half'; }
			if (total==3) { gridWidth = 'medium-up--one-third'; }
			if (total==4) { gridWidth = 'medium--one-half large-up--one-quarter'; }
			var gridCol			= '<div class="grid__item '+gridWidth+' '+imgCls+'">';
			$(el).each(function(index) {
				stripUrl 			= $(this).attr('src');
				begin				= gridCol;
				end					= gridEnd;
				stripAlt 			= $(this).attr('alt');
				caption 			= '';
				title 				= '';
				isCaption			= stripAlt.indexOf("CAPTION") >= 0;
				if ( isCaption == false && stripAlt != '' ) {
					var updatedAlt 	= stripAlt.trim();
					title 			= 'title="'+updatedAlt+'"';
				}
				if ( isCaption == true && stripAlt != '' ) {
					var updatedAlt 	= stripAlt.replace("CAPTION", "").trim();
					caption 		= '<div class="caption">'+updatedAlt+'</div>';
				}
				bgImgRplc 			= begin+'<div class="bg_image '+imgSize+'" '+title+' style="background-image: url('+stripUrl+');"></div>'+caption+end; 
				$(this).replaceWith(bgImgRplc);
			})
			$('.'+imgCls).wrapAll(grid);
			$('.'+contCls).unwrap();
		}
